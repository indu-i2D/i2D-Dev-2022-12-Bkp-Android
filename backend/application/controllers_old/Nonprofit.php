...
class Nonprofit extends CI_Controller {
	...
	public function sub_category_list(){
		...
		else
	    {
	        ...
	        $category = $this->Nonprofit_model->getMainCategory($category_id);
            $data['category_id'] = $category_id;
			$data['maincategory'] = $this->Nonprofit_model->getSubCategoryList($category[DB_prefix.'CATEGORY_CODE']);
			$this->load->view('common/header', $data);
			$this->load->view('non_profit/sub_category_list', $data);
			$this->load->view('common/footer');
	    }
	}
	...
	public function child_category_list(){
		...
		else
	    {
	        ...
	        $data['title']="Child Category List";
	        $category = $this->Nonprofit_model->getMainSubCategory($subcategoryid);
            $data['category_id'] = $category_id;
             $data['subcategory_id'] = $subcategoryid;
			$data['maincategory'] = $this->Nonprofit_model->getChildCategoryList($category[DB_prefix.'NTEE_1_CODE']);
			$this->load->view('common/header', $data);
			$this->load->view('non_profit/child_category_list', $data);
			$this->load->view('common/footer');
	    }
	}
	...
	public function view()
	{
		if (!$this->session->userdata('admin'))
		{
			...
			$data['non_profit_list'] = $this->Nonprofit_model->getNonProfitList();
			$this->load->view('common/header', $data);
    ...
    public function delete_subcategory()
	{
		if (!$this->session->userdata('admin'))
		{
			...
			$this->db->where(DB_prefix.'NTEE_1_ID', $sub_category_id);
		    $this->db->delete(DB_prefix."NTEE_1");
		    $this->session->set_flashdata('message', 'Data has been deleted');
			//redirect(base_url()."nonprofit/sub_category_list/".$category_id);
			redirect(base_url()."nonprofit/category_list");
		}
	}
	 ...
	 public function delete_child_category()
	{
		if (!$this->session->userdata('admin'))
		{
			...
			$category_id = $this->uri->segment(3);
			$sub_category_id = $this->uri->segment(4);
			$child_category_id = $this->uri->segment(5);
			$this->db->where(DB_prefix.'NTEE_2_ID', $child_category_id);
		    $this->db->delete(DB_prefix."NTEE_2");
		    $this->session->set_flashdata('message', 'Data has been deleted');
			//redirect(base_url().'nonprofit/child_category_list/'.$category_id.'/'.$sub_category_id);
			redirect(base_url()."nonprofit/category_list");
		}
	}
	...
	public function edit_sub_category()
	{
		if (!$this->session->userdata('admin'))
		{
			...
			if($_POST){
				$update_data = array(
				       DB_prefix.'NTEE_1_NAME' => $_POST['name'],
				       DB_prefix.'NTEE_1_CODE'   => $_POST['code'],
				       DB_prefix.'NTEE_1_DESCRIPTION' =>  $_POST['description'],
				       DB_prefix.'ACTIVE' => $_POST['status']
				);
			    $this->db->where(DB_prefix.'NTEE_1_ID', $sub_category_id);
			    $this->db->update(DB_prefix."NTEE_1", $update_data);
			    $this->session->set_flashdata('message', 'Data has been updated');
			    //redirect(base_url()."nonprofit/sub_category_list/".$category_id);
			    redirect(base_url()."nonprofit/category_list");
			}
			    ...
			    $data['title']="Edit Sub Category";
    			$data['category'] = $this->Nonprofit_model->getMainSubCategory($sub_category_id);
    			$this->load->view('common/header', $data);
    			$this->load->view('non_profit/edit_sub_category', $data);
    			$this->load->view('common/footer');
			}
		}
	}
	...
	public function add_sub_category()
	{
		if (!$this->session->userdata('admin'))
		{
			...
			if($_POST){
				$maincategory = $this->db->query("SELECT * from ".DB_prefix."NTEE_1 WHERE ".DB_prefix."NTEE_1_CODE = '".$_POST['code']."' AND ".DB_prefix."CATEGORY_ID = ".$_POST['category_id']);
                if($maincategory->num_rows() == 0)
                {
					$insert_data = array(
					       DB_prefix.'NTEE_1_NAME' => $_POST['name'],
					       DB_prefix.'NTEE_1_CODE'   => $_POST['code'],
					       DB_prefix.'CATEGORY_ID' => $_POST['category_id'],
					       DB_prefix.'NTEE_1_DESCRIPTION' =>  $_POST['description'],
					       DB_prefix.'ACTIVE' => $_POST['status']
					);
				    $this->db->insert("i2D_D_NTEE_1", $insert_data);
				    $this->session->set_flashdata('message', 'Data has been inserted');
				    redirect(base_url()."nonprofit/category_list");
				    //redirect(base_url()."nonprofit/sub_category_list/".$category_id);
				}
				...
				{
					$this->session->set_flashdata('error', 'Sub Category Code Already Exists');
					redirect(base_url()."nonprofit/category_list");
				    //redirect(base_url()."nonprofit/sub_category_list/".$category_id);
				}
			}
			    ...
			    $data['maincategory'] = $this->Nonprofit_model->getCategoryList();
    			$this->load->view('common/header', $data);
    			$this->load->view('non_profit/add_sub_category', $data);
    			$this->load->view('common/footer');
			}
		}
	}
	...
	public function add_child_category()
	{
		if (!$this->session->userdata('admin'))
		{
			...
			$data['title']="Add Child Category";
			if($_POST){
				$maincategory = $this->db->query("SELECT * from ".DB_prefix."NTEE_2 WHERE ".DB_prefix."NTEE_2_CODE = '".$_POST['code']."' AND ".DB_prefix."CATEGORY_ID = ".$_POST['category_id']." AND ".DB_prefix."NTEE_1_ID = ".$_POST['sub_category_id']);
				if($maincategory->num_rows() == 0)
                {
					$insert_data = array(
					       DB_prefix.'NTEE_2_NAME' => $_POST['name'],
					       DB_prefix.'NTEE_2_CODE'   => $_POST['code'],
					       DB_prefix.'CATEGORY_ID' => $_POST['category_id'],
					       DB_prefix.'NTEE_1_ID' => $_POST['sub_category_id'],
					       DB_prefix.'NTEE_2_DESCRIPTION' =>  $_POST['description'],
					       DB_prefix.'ACTIVE' => $_POST['status']
					);
				    $this->db->insert("i2D_D_NTEE_2", $insert_data);
				    $this->session->set_flashdata('message', 'Data has been inserted');
				    redirect(base_url()."nonprofit/category_list");
				    //redirect(base_url().'nonprofit/child_category_list/'.$category_id.'/'.$sub_category_id);
				}
				...
				{
					$this->session->set_flashdata('error', 'Child Category Code Already Exists');
					redirect(base_url()."nonprofit/category_list");
				}
			}
			else {
			    ...
			    $data['maincategory'] = $this->Nonprofit_model->getCategoryList();
    			$this->load->view('common/header', $data);
    			$this->load->view('non_profit/add_child_category', $data);
    			$this->load->view('common/footer');
			}
		}
	}
	 ...
	 public function edit_child_category()
	{
		if (!$this->session->userdata('admin'))
		{
			$this->load->view('user/login');
			...
			if($_POST){
			    ...
			    $this->db->where(DB_prefix.'NTEE_2_ID', $child_category_id);
			    $this->db->update(DB_prefix."NTEE_2", $update_data);
			    $this->session->set_flashdata('message', 'Data has been updated');
			    //redirect(base_url().'nonprofit/child_category_list/'.$category_id.'/'.$sub_category_id);
			    redirect(base_url()."nonprofit/category_list");
			}
			    ...
			    $data['title']="Edit Child Category";
    			$data['category'] = $this->Nonprofit_model->getMainChildCategory($child_category_id);
    			$this->load->view('common/header', $data);
    			$this->load->view('non_profit/edit_child_category', $data);
    			$this->load->view('common/footer');
			}
		}
	}
    ...
    public function updateAddress($id, $street, $city, $state, $zip)
    {
		...
		$url = "https://maps.googleapis.com/maps/api/geocode/json?address=".$address."&key=AIzaSyDfOZf_bITlewuOVvnVICvjQ70VdFHmR2Q";
		$ch = curl_init($url);
			...
			if(isset($_FILES["file"]["name"]))
			{
				...
				foreach($object->getWorksheetIterator() as $worksheet)
				{
					...
					for($row=2; $row<=$highestRow; $row++)
					{
						...
						if(!empty($category_code) && $sub_category_code=='' && $child_category_code=='')
						{
						...
						else if(!empty($category_code) && (!empty($sub_category_code) || $sub_category_code == '0') && $child_category_code == '')
						{
							$testcount = strlen($category_code);
					    ...
					    else if(!empty($category_code) && (!empty($sub_category_code) || $sub_category_code =='0') && (!empty($child_category_code) || $child_category_code == '0'))
						{
							$testcount = strlen($category_code);
							$testcount1 = strlen($sub_category_code);
	...
	public function category_bulk_upload()
	{
		...
		$path = $_SERVER['DOCUMENT_ROOT'].'/i2donate/category_bulk_upload.xlsx';
		$object = PHPExcel_IOFactory::load($path);
		foreach($object->getWorksheetIterator() as $worksheet)
		{
			$highestRow = $worksheet->getHighestRow();
			...
			{
				$category_code = ucfirst(strtolower(trim($worksheet->getCellByColumnAndRow(0, $row)->getValue())));
				$sub_category_code = ucfirst(strtolower(trim($worksheet->getCellByColumnAndRow(1, $row)->getValue())));
				$child_category_code = ucfirst(strtolower(trim($worksheet->getCellByColumnAndRow(2, $row)->getValue())));
				$category = ucfirst(strtolower(trim($worksheet->getCellByColumnAndRow(4, $row)->getValue())));
				$description = ucfirst(strtolower(trim($worksheet->getCellByColumnAndRow(5, $row)->getValue())));
				...
				if($category_code!=""){
					...
					$this->db->insert("i2D_D_types", $insert_data);
					$category_id = $this->db->insert_id();
				}
				if($sub_category_code!=""){
					$insert_data = array(
						'i2D_D_SUB_CATEGORY_CODE'  => trim($sub_category_code),
						'i2D_D_CATEGORY_ID'  => $category_id,
						'i2D_D_SUB_CATEGORY_NAME'   => trim($category),
						'i2D_D_SUB_CATEGORY_DESCRIPTION'    => trim($description),
						'i2D_D_ACTIVE' => 'Yes'
					);
					$this->db->insert("i2D_D_sub_types_1", $insert_data);
					$sub_category_id = $this->db->insert_id();
				}

				if($child_category_code!=""){
					$insert_data = array(
						'i2D_D_CHILD_CATEGORY_CODE'  => trim($child_category_code),
						'i2D_D_CATEGORY_ID'  => $category_id,
						'i2D_D_SUB_CATEGORY_ID'  => $sub_category_id,
						'i2D_D_CHILD_CATEGORY_NAME'   => trim($category),
						'i2D_D_CHILD_CATEGORY_DESCRIPTION'    => trim($description),
						'i2D_D_ACTIVE' => 'Yes'
					);
					$this->db->insert("i2D_D_sub_types_2", $insert_data);
					$child_category_id = $this->db->insert_id();
				}
...
public function get_category_subcategory()
{
		...
		else
		{
				...
				$return_data = $this->Nonprofit_model->get_category_subcategory();
				echo "<pre>";
				print_r($return_data);
				exit;
